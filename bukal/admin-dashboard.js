...
// after dashboard.js.
  ...
  const ACADEMIC_ADMIN_TILE_KEYS = new Set([
    ...
    'programsSubjects', 'timetableConflicts', 'attendanceConfig', 'quranTracker',
  ...
  // Taqdim Assistant / Translation Service / Quran Tracker plus every
  // Academics-group + Attendance tile are all school-level features now,
  ...
  // Taqdim/Translation default OFF (subscription-gated); the Academics
  // items and Quran Tracker default ON (School::ACADEMIC_FEATURES) so an
  // existing school sees no change until a SuperAdmin explicitly flips
  // one off. Either way, a school with none of a group's items enabled
  // simply gets no group at all (empty items are dropped by the
  // s.items.length > 0 filter below) - nothing to keep in sync beyond
  // reading `features[key]` here.
  function buildSections(user) {
    const features = (user && user.school_features) || {};
    const on = key => features[key] !== false; // ACADEMIC_FEATURES default true - absence/undefined still shows the tile
    const serviceItems = [];
    if (features.taqdim) {
    ...
    const academicItems = [
      ...
      { key: 'quranTracker', title: t('admin_dashboard.quran_tracker_title', 'Quran Tracker'), desc: t('admin_dashboard.quran_tracker_desc', 'Track Quran memorization (Hifz) progress'), icon: 'book', tint: TINT.teal, href: 'quran-tracker-dashboard.php' },
    ].filter(i => on(i.key));
    const activityItems = [
    ...
    return [
      ...
      { label: t('admin_dashboard.group_academics', 'Academics'), items: academicItems },
      { label: t('admin_dashboard.group_activity', 'Activity & Requests'), items: activityItems },
...
// ── Taqdim/Translation pending-requests cards — visible to EVERY admin at
// this school, not just whoever is currently assigned. Any free admin can
// see at a glance that unassigned requests are waiting and jump straight
// to the unassigned queue (taqdim-translation-applications.php?...&
// unassigned=1) to grab one - there is no per-admin "claim" lock, so
// whichever admin gets there first can assign it to themself immediately
// (see taqdim-translation-applications.js's openAssignSheet). Counts are a
// nice-to-have, not load-blocking - a failed fetch just leaves the card
// absent for that feature, same as the subscription/sync cards' failure
// handling above.
function pendingRequestsCardHtml(feature, count) {
  if (!count || count <= 0) return '';
  const title = feature === 'taqdim'
    ? t('admin_dashboard.taqdim_title', 'Taqdim Assistant')
    : t('admin_dashboard.translation_title', 'Translation Service');
  return (
    '<a class="status-card dark" href="taqdim-translation-applications.php?feature=' + feature + '&unassigned=1">' +
      '<span class="status-card-icon" style="background:linear-gradient(135deg,#F59E0B,#B45309);">' + icon('inbox', { size: 20, color: '#fff' }) + '</span>' +
      '<span class="status-card-body">' +
        '<span class="status-card-title-row">' +
          '<span class="status-card-title">' + escapeHtml(title) + '</span>' +
          '<span class="status-pill" style="background:rgba(255,255,255,0.08);">' +
            '<span class="status-pill-dot" style="background:#F59E0B;"></span>' +
            '<span class="status-pill-text" style="color:#F59E0B;">' + escapeHtml(t('admin_dashboard.unassigned_pill', 'Unassigned')) + '</span>' +
          '</span>' +
        '</span>' +
        '<div class="status-card-subtitle">' + escapeHtml(t('admin_dashboard.pending_requests_subtitle', '{count} waiting - any admin can assign').replace('{count}', count)) + '</div>' +
      '</span>' +
      '<span class="status-card-badge">' + count + '</span>' +
    '</a>'
  );
}

let pendingRequestsState = { taqdim: 0, translation: 0 };
function loadPendingRequestsCards(token, user) {
  const features = (user && user.school_features) || {};
  const fetches = [];
  ...
  if (features.taqdim) {
    fetches.push(
      fetchAdminTaqdimTranslationApplications(token, { feature: 'taqdim', status: 'submitted' })
        .then(list => { pendingRequestsState.taqdim = list.filter(a => !a.assigned_to).length; })
        .catch(() => {})
    );
  }
  if (features.translation) {
    fetches.push(
      fetchAdminTaqdimTranslationApplications(token, { feature: 'translation', status: 'submitted' })
        .then(list => { pendingRequestsState.translation = list.filter(a => !a.assigned_to).length; })
        .catch(() => {})
    );
  }
  if (fetches.length === 0) return;
  Promise.all(fetches).then(renderStatusCards);
}
...
function renderStatusCards() {
  ...
  wrap.innerHTML =
    pendingRequestsCardHtml('taqdim', pendingRequestsState.taqdim) +
    pendingRequestsCardHtml('translation', pendingRequestsState.translation) +
...
window.addEventListener('muslimedu:synced', refreshSyncCard);
  ...
  guardDashboard('admin', function (user, token) {
    ...
    refreshSyncCard();
    loadPendingRequestsCards(token, user);
