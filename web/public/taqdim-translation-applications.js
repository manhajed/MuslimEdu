...
// school's admin team rarely needs one).
//
...
function renderDetailContent(token, app) {
  ...
  return (
    ...
    '<div class="chip-row" style="margin-bottom:14px;">' +
      ...
      (app.assignee ? '<span class="mini-chip ok">' + escapeHtml(t('taqdim_translation_applications.assigned_chip', '→ {name}').replace('{name}', app.assignee.name)) + '</span>' : '') +
    '</div>' +
    ...
    (app.assigned_to
      ? '<a href="chat-box.php?userId=' + encodeURIComponent(app.student.id) + '&name=' + encodeURIComponent(studentName) + '" class="sheet-btn-secondary" style="width:100%;margin-top:10px;text-decoration:none;display:block;text-align:center;">' + escapeHtml(t('taqdim_translation_applications.message_student_btn', 'Message Student')) + '</a>'
      : '') +
